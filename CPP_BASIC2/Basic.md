...
## 생성자(Constructor)와 소멸자(Distructor)
 ...
 # 다형성
  ...
  # 가상함수 
   - 부모클래스, 자식클래스의 상속 관계에서 필요한 개념/기법
   - 오버라이딩 : 동일한 이름의 함수 중 부모클래스에 있는 함수가
   - 부모클래스에 있는 함수를 자식 클래스에서 오버라이딩 될 것을 예측해서 정적으로 구현
   - 부모클래스에 있는 함수를 동적 생성하도록 가상 함수로 구현
   - 

   # 순수 가상함수
   - 함수의 선언만 있는 것(기능 구현이 안되어 있는 것) 
   - 부모 클래스를 단독으로 사용하지 않는 것을 의미
   - 순수 가상함수가 있는 부모클래스를 상속받은 자식클래스는 반드시
   - 순수 가상함수를 오버라이딩해서 사용해야함
   - 순수 가상함수를 포함하는 클래스 : 추상클래스
   - 추상클래스는 단독으로 객체로 생성할 수 없고 상
   - 

   # this
    - Aminal class 참고
    - this가 포함된 클래스 객체를 가리킴
    - this포인터는 ->를 사용해서 멤버 변수를 표시
